...
import { mockAnnouncementList, mockServiceList } from "../src/lib/mockData";
import type {
  Announcement,
...
export const publicApi = {
  getAnnouncements: async (
    ...
    const response = await apiClient.get(
      `/api/public/announcements?page=${page}&limit=${limit}`
    );
  ...
  getAnnouncement: async (id: number): Promise<{ data: Announcement }> => {
    if (USE_MOCK_DATA) {
      ...
      const announcement = mockAnnouncementList.data.find((item) => item.id === id);
      if (!announcement) {
