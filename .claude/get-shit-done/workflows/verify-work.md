...
<template>
@D:/capstone-hsms/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
Commit the UAT file:
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @D:/capstone-hsms/.claude/get-shit-done/workflows/diagnose-issues.md
