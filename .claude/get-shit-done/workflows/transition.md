...
```bash
TRANSITION=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
