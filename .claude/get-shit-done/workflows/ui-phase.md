...
```bash
UI_RESEARCHER_MODEL=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```markdown
Read D:/capstone-hsms/.claude/agents/gsd-ui-researcher.md for instructions.
...
Template: D:/capstone-hsms/.claude/get-shit-done/templates/UI-SPEC.md
</output>
...
```markdown
Read D:/capstone-hsms/.claude/agents/gsd-ui-checker.md for instructions.
...
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
