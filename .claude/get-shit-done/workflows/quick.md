...
```bash
INIT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
