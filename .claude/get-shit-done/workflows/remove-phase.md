...
```bash
INIT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
