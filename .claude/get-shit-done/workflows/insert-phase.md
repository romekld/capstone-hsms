...
```bash
INIT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
