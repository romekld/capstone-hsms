...
```bash
AUDITOR_MODEL=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read D:/capstone-hsms/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `D:/capstone-hsms/.claude/get-shit-done/templates/VALIDATION.md`
...
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
