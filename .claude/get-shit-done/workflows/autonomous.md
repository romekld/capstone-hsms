...
```bash
DETAIL=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
node "D:/capstone-hsms/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
