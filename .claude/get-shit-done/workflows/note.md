...
- **Global scope**: `D:/capstone-hsms/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/capstone-hsms/.claude/notes/`)
...
2. Glob `D:/capstone-hsms/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/capstone-hsms/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/capstone-hsms/.claude/notes/` — works in any directory
